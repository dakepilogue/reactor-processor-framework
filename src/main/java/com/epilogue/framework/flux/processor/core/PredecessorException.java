...
/**
 * 表示当前 IProcessor 因为之前的 IProcessor 没有正确返回结果，发生的异常. 之前的 IProcessor 的错误将被包含在 causes 中
 */
@Getter
public class PredecessorException extends RuntimeException {

    private final Throwable[] causes;
    private final Class<? extends IProcessor> processor;

    /**
     * 使用完整的参数创建.
     */
    public PredecessorException(String message, Throwable[] causes,
        Class<? extends IProcessor> processor) {
     ...
     */
    public PredecessorException(Throwable[] causes, Class<? extends IProcessor> processor) {
        super(String.format(
