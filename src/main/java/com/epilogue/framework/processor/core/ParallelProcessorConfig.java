...
import static com.epilogue.framework.processor.util.Keys.CHILDREN;
import static com.epilogue.framework.processor.util.Keys.LATCH;
...
class ParallelProcessorConfig extends ProcessorConfig {
    ...
    ParallelProcessorConfig(Config config) {
        ...
        this.childrenConfigs = config.getConfigList(CHILDREN).stream()
            .map(c -> ProcessorConfig.create(c))
