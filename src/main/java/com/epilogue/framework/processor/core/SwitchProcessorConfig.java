...
import static com.epilogue.framework.processor.util.Keys.CASES;
import static com.epilogue.framework.processor.util.Keys.CONDITION;
import static com.epilogue.framework.processor.util.Keys.CONDITION_VALUE;
...
class SwitchProcessorConfig extends ProcessorConfig {
    ...
    SwitchProcessorConfig(Config config) {
        ...
        this.conditionConfig = ProcessorConfig.create(config.getConfig(CONDITION));
        final Config cases = config.getConfig(CASES);
        this.casesConfigMap = config.getObject(CASES).keySet().stream()
            .collect(Collectors.toMap(
                key -> key,
                key -> ProcessorConfig.create(cases.getConfig(key))
            ));
    }
